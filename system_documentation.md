...
- The backend will attempt to connect to the MongoDB instance specified by
  `MONGO_URI`. If a real MongoDB server is not available, the application
  falls back to an in-memory `mongomock` instance for development and demo
  purposes. This means local WiredTiger DB folders (e.g. `mongo_data/`) are
  not required by the code and should not be committed to the repository.
...
- Vercel: Make sure `vercel.json` is present and routes `/` to `/frontend/`.
  Set environment variables in the Vercel Dashboard and ensure any `/api`
  calls are proxied to your backend service or routed through serverless
  functions if you choose to host the API on Vercel.
- Railway: Use the repository root `Procfile` (`web: gunicorn backend.app:app --bind 0.0.0.0:$PORT`).
  Add environment variables in Railway's Environment section. If you prefer
  placing the `Procfile` in `backend/`, configure Railway to use that folder
  as the project root.
